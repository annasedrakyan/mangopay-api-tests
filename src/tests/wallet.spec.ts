...
import { createApiContext, createUser, createWallet, fetchExistingWalletIds, getBearerToken } from '../commons/requests';
...
//     });
    ...
    test('should create a valid wallet for an existing user and ensure wallet ID is unique', async () => {
      const apiContext = await createApiContext();
      
      const userResponse = await createUser(apiContext, testData.validPayerUser);
      const userResponseBody = await userResponse.json();

      const existingWalletIds = await fetchExistingWalletIds(apiContext, userResponseBody.Id);
    
      const validWallet = {
        ...testData.wallet,
        Owners: [userResponseBody.Id],
      };
    
      const walletResponse = await createWallet(apiContext, validWallet);
      const wallet = await walletResponse.json();
      const newWalletId = wallet.Id;
    
      expect(existingWalletIds).not.toContain(newWalletId);
    });
});  
